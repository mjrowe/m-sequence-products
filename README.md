...
```
in terms of some primitive element $\alpha \in \text{GF}(2^n)$. It also covers the link to alternative definitions: polynomials over $`\{0,1\}`$ and linear feedback shift registers, something that is not always clear in the existing literature.
...
```
The offsets are the Zech logarithms of $\text{GF}(2^n)$ corresponding to the primitive element $\alpha$. This is relevant in the modelling of non-linear systems as the Volterra series
```math
...
## Code
The program `main.py` calculates product of a user supplied m-sequence with all possible shifted versions of itself (i.e. all $\tau$). As the circular autocorrelation of an m-sequence is an impulse (see the notes for a beautiful proof!) one can find the resulting offset $Z_\alpha(\tau)$ by the delay of this peak. The program thus essentially calculates the Zech logarithms of $\text{GF}(2^n)$ for the primitive element that corresponds to the m-sequence. This is not a particularly efficient method of doing this calculation but rather pedagogical.
...
No artificial intelligence tools were used in the creation of this repository, any mistakes are entirely mine!
